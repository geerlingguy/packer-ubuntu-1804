...
**Current Ubuntu Version Used**: 18.04

**Pre-built Vagrant Box**:

  - [`vagrant init geerlingguy/ubuntu1804`](https://vagrantcloud.com/geerlingguy/boxes/ubuntu1804)

This example build configuration installs and configures Ubuntu 18.04 x86_64 minimal using Ansible, and then generates two Vagrant box files, for:
...
If you want to only build a box for one of the supported virtualization platforms (e.g. only build the VMware box), add `--only=vmware-iso` to the `packer build` command:
    ...
    $ packer build --only=vmware-iso ubuntu1804.json
